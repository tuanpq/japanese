...
class GrammarUseCase @Inject constructor(
    ...
    suspend fun findAllGrammars() : Flow<UIState<List<GrammarEntity>>> {
        return grammarRepository.findAllGrammars().map { result ->
            when (result) {
                is APIState.Success -> {
                    UIState.Success(grammarMapper.mapList(result.data))
                }
    ...
    suspend fun findGrammarById(grammarId: Int) : Flow<UIState<GrammarEntity>> {
        return grammarRepository.findGrammarById(grammarId).map { result ->
            when (result) {
                is APIState.Success -> {
                    UIState.Success(grammarMapper.mapObject(result.data))
                }
                is APIState.Error -> UIState.Error(result.message)
            }
        }
